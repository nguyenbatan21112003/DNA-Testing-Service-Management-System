...
const OrderManagement = () => {
      ...
      <div style={{ padding: 8 }}>
        ...
        >
          ...
          {order.type && (
            <Tag
              style={{
                background: order.type.toLowerCase().includes("hành chính")
                  ? "#36cfc9"
                  : "#722ed1",
                color: "#fff",
                fontWeight: 700,
            ...
            >
              {order.type.toLowerCase().includes("hành chính")
                ? "Hành chính"
                : order.type.toLowerCase().includes("dân sự")
                ? "Dân sự"
                : "Khác"}
            </Tag>
          )}
        ...
        >
          ...
          >
            <div>
              <strong>Loại xét nghiệm:</strong> {order.type}
            </div>
          </div>
          {/* Bảng mẫu xét nghiệm */}
          <div
                ...
                <tbody>
                  {sampleData.map((data, index) => (
                    ...
                    >
                      ...
                      >
                        {data.gender || data.gioiTinh || data.gioitinh || ""}
                      </td>
                      <td
                        style={{
                          padding: "10px 14px",
                      ...
                      >
                        {data.sampleType || data.loaiMau || data.loaimau || ""}
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        >
          ...
          {order.conclusion ? (
            <div style={{ fontSize: 18, color: "#005c3c", fontWeight: 700 }}>
              {order.conclusion}
            </div>
          ) : (
            <div style={{ color: "#faad14", fontWeight: 600 }}>
  ...
  const renderSimpleDetailModal = (order) => (
    <div style={{ padding: 8 }}>
      ...
      >
        ...
        <div style={{ fontSize: 16 }}>
          <b>Loại xét nghiệm:</b> {order.type}
        </div>
        {order.date && (
          <div style={{ fontSize: 16 }}>
            <b>Ngày hẹn:</b> {order.date}
          </div>
        )}
      </div>
    </div>
  ...
  const myOrders = orders.filter((o) => {
    const staffAssigned = (o.staffAssigned || "")
      .toString()
      .trim()
      .toLowerCase();
  ...
  // Thống kê
  const stats = {
    waiting: orders.filter((o) => getStatusText(o.status) === "Chờ xác nhận")
      .length,
    my: myOrders.length,
    completed: orders.filter((o) => getStatusText(o.status) === "Hoàn thành")
      .length,
  };
    ...
    // Thêm cột phân loại
    {
      ...
      render: (_, record) => {
        ...
        if (record.type && record.type.toLowerCase().includes("hành chính"))
          label = "Hành chính";
        else if (record.type && record.type.toLowerCase().includes("dân sự"))
          label = "Dân sự";
        return label ? (
          <Tag color={label === "Dân sự" ? "#722ed1" : "#36cfc9"}>{label}</Tag>
        ) : null;
      },
    },
    {
      ...
      <Card>
        <Tabs activeKey={activeTab} onChange={setActiveTab}>
          <TabPane tab={`Đơn chờ xác nhận (${stats.waiting})`} key="waiting">
            <Table
              columns={columnsWithActionWaiting}
          ...
          title={
            viewOrder
              ? getStatusText(viewOrder.status) === "Hoàn thành"
                ? null
                : SIMPLE_DETAIL_STATUSES.includes(
            ...
            (getStatusText(viewOrder.status) === "Hoàn thành" ? (
              ...
              <Descriptions bordered column={1}>
                ...
                <Descriptions.Item label="Phân loại">
                  {viewOrder.type &&
                  viewOrder.type.toLowerCase().includes("hành chính")
                    ? "Hành chính"
                    : viewOrder.type &&
                      viewOrder.type.toLowerCase().includes("dân sự")
                    ? "Dân sự"
                    : "Khác"}
                </Descriptions.Item>
                <Descriptions.Item label="Phương thức lấy mẫu">
                  {viewOrder.sampleMethod === "home" ? "Tại nhà" : "Tại cơ sở"}
                </Descriptions.Item>
