  ...
  useEffect(() => {
    ...
    if (prefill) {
      ...
      if (data.orderId) {
        ...
        const order = orders.find(o => o.id.toString() === data.orderId.toString());
        if (order) {
          setSelectedOrder(order);
          form.setFieldsValue({
            collectionDate: order.appointmentDate ? dayjs(order.appointmentDate) : undefined,
            ...
            >
              ...
              <Row gutter={16}>
                ...
                <Col span={12}>
                  ...
                  >
                    <Input value={selectedOrder ? `#${selectedOrder.id}` : ""} disabled style={{ fontWeight: 700, color: "#0984e3" }} />
                  </Form.Item>
                </Col>
              </Row>
              ...
              {selectedOrder && (
                <Row gutter={16}>
                  <Col span={12}>
                    <Form.Item name="testType" label="Loại xét nghiệm" rules={[{ required: true, message: "Vui lòng chọn loại xét nghiệm!" }]}> 
                      <Select
                        value={form.getFieldValue("testType")}
                        onChange={value => form.setFieldsValue({ testType: value })}
                      >
              ...
              <Row gutter={16}>
                ...
                <Col span={12}>
                  ...
                  >
                    <Input
                      placeholder="Địa chỉ hiện tại của người yêu cầu"
                    />
                  </Form.Item>
                </Col>
              </Row>
                  ...
                  <Row gutter={16}>
                    ...
                    <Col span={6}>
                      <Form.Item label="Giới tính" required>
                        <Select
                          value={donor.gender}
                          onChange={(value) => updateDonor(donor.id, "gender", value)}
                        >
                      ...
                      <Form.Item label="Vân tay">
                        <Upload
                          ...
                          beforeUpload={file => {
                            ...
                            reader.onload = e => {
                              updateDonor(donor.id, "fingerprint", e.target.result);
                            };
                        ...
                        >
                          {donor.fingerprint ? (
                            <img src={donor.fingerprint} alt="fingerprint" style={{ width: '100%', maxHeight: 80, objectFit: 'contain' }} />
                          ) : (
                            <div>
                              <PlusOutlined />
                        ...
                        {donor.fingerprint && (
                          <Button danger size="small" style={{ marginTop: 4 }} onClick={() => updateDonor(donor.id, "fingerprint", null)}>
                            Xóa ảnh
                          </Button>
                        )}
                      </Form.Item>
