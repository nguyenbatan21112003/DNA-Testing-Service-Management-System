...
function mapOrderStatus(status) {
  ...
  if ([
    'kit_not_sent', 'kit_sent', 'sample_received', 'processing', 'completed'
  ].includes(s.toUpperCase())) return s.toUpperCase();
  return 'KIT_NOT_SENT';
}
...
const HomeSampling = () => {
  ...
  const loadSamplingRequests = () => {
    ...
    const homeSamplingOrders = allOrders
      .filter((order) => order.sampleMethod === "home" && !order.isHiddenByStaff && order.staffAssigned)
      .map((order) => {
        // Ưu tiên lấy status mới nhất từ status, kitStatus, samplingStatus
        const mappedStatus = mapOrderStatus(order.status || order.kitStatus || order.samplingStatus);
        return {
          ...order,
          status: mappedStatus,
  ...
  const handleProcessing = async (record) => {
    ...
    if ((!sampleInfo.donors || sampleInfo.donors.length === 0) &&
        record.members && Array.isArray(record.members) && record.members.length > 0) {
  ...
  const stats = {
    ...
    kitNotSent: samplingRequests.filter(r => r.status === 'KIT_NOT_SENT').length,
    kitSent: samplingRequests.filter(r => r.status === 'KIT_SENT').length,
    sampleReceived: samplingRequests.filter(r => r.status === 'SAMPLE_RECEIVED').length,
    processing: samplingRequests.filter(r => r.status === 'PROCESSING').length,
  ...
  return (
    <div style={{ padding: 24, background: "#f5f5f5", minHeight: "100%" }}>
        ...
        {selectedRequest && (
          <div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <p><strong>Họ tên:</strong> {selectedRequest.name}</p>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <p><strong>Mã kit:</strong> {selectedRequest.kitId || 'Chưa cấp mã kit'}</p>
              <p>
                <strong>Trạng thái hiện tại:</strong>{' '}
                <Tag color={getStatusColor(selectedRequest.status)} style={{ fontWeight: 600, fontSize: 15 }}>
              ...
              >
                <Step
                  ...
                  status={(() => {
                    ...
                    if (["PENDING_CONFIRM", "KIT_NOT_SENT", "KIT_SENT", "SAMPLE_RECEIVED", "PROCESSING", "COMPLETED", "Hoàn thành"].includes(s)) return "finish";
                    return "wait";
                  })()}
                />
                <Step
                  ...
                  status={(() => {
                    const s = selectedRequest.status;
                    if (["KIT_SENT", "SAMPLE_RECEIVED", "PROCESSING", "COMPLETED", "Hoàn thành"].includes(s)) return "finish";
                    if (["PENDING_CONFIRM", "KIT_NOT_SENT"].includes(s)) return "wait";
                    return "wait";
                  })()}
                />
                <Step
                  ...
                  status={(() => {
                    const s = selectedRequest.status;
                    if (["SAMPLE_RECEIVED", "PROCESSING", "COMPLETED", "Hoàn thành"].includes(s)) return "finish";
                ...
                <Step
                  ...
                  status={(() => {
                    ...
                    if (["COMPLETED", "Hoàn thành"].includes(s)) return "finish";
                    return "wait";
                  })()}
                />
              </Steps>
      ...
      <Modal
        ...
        width={600}
      >
        <Form form={form} layout="vertical" onFinish={(values) => handleSendKit(selectedRequest, values)}>
