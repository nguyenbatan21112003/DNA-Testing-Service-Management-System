...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      ...
      // Thêm tài khoản Manager mẫu vào danh sách users
      const mockUsers = [
        ...res.data,
        {
          user_id: 999,
  ...
  const requestPasswordChange = (currentPassword, newPassword) => {
    ...
    sessionStorage.setItem(`otp_${user.email}`, JSON.stringify({ otp, newPassword, created: Date.now() }))
