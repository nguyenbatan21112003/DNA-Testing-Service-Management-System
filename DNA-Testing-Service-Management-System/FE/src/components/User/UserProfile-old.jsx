...
const UserProfile = () => {
  ...
  const fetchResult = async () => {
    try {
      const res = await customerApi.getResults(user?.userId);
  ...
  useEffect(() => {
    const loadUserOrders = async () => {
      try {
        ...
        const [resOrders, resResults] = await Promise.all([
          customerApi.getTestRequest(),
          fetchResult(),
        ]);

        const orders = Array.isArray(resOrders.data) ? resOrders.data : [];
        const results = Array.isArray(resResults) ? resResults : [];

        const ordersWithResult = await Promise.all(
          orders.map(async (order) => {
            let process = null;
            try {
              const response = await customerApi.getTestProcessByRequestId(
                order.requestId
              );
            ...
            const matchedResult = results.find(
              (r) => r.processId === process?.processId
            );

            return {
              id: order.requestId,
              processId: process?.processId,
              type: order.serviceName,
              status:
                process?.currentStatus?.toUpperCase() ||
                order.status?.toUpperCase(),
              date: order.createdAt?.split("T")[0] || "",
              appointmentDate: order.scheduleDate?.split("T")[0] || "",
              kitCode: process?.kitCode || "",
              claimedAt: process?.claimedAt || "",
              updatedAt: process?.updatedAt || "",
              notes: process?.notes || "",
              result: matchedResult || null, // gán kết quả nếu có
            };
          })
        );

        setUserOrders(ordersWithResult);
      } catch (err) {
        console.error("Lỗi khi tải đơn hoặc kết quả:", err);
      }
  ...
  return (
    ...
    >
                          ...
                          >
                            ...
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  const res =
                                    await customerApi.getDeclarantByRequestId(
                                      order.id
                                    );
                                  const declarant = res?.data;
                                  ...
                                  console.error(
                                    "Lỗi khi lấy thông tin declarant:",
                                    err
                                  );
                                  message.error(
                                    "Không lấy được thông tin người khai báo"
                                  );
                                }
                              }}
                            >
                            ...
                            {(getStatusText(getDisplayStatus(order)) ===
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  try {
                                    const res =
                                      await userApi.getDeclarantByRequestId(
                                        order.id
                                      );
                                    const declarant = res?.data;
          ...
          {tab === "settings" && (
            ...
            >
              <form
                ...
                onSubmit={handleChangePassword} // ✅ Sửa tại đây
              >
      ...
      {showFeedbackModal &&
        selectedOrder &&
        (getStatusText(getDisplayStatus(selectedOrder)) === "Đã có kết quả" ||
          getStatusText(getDisplayStatus(selectedOrder)) === "Hoàn thành") && (
          <div
            style={{
              position: "fixed",
              top: 0,
            ...
            selectedOrder.type.toLowerCase().includes("dân sự") ? (
              <>
                ...
                {selectedOrder.conclusion && (
                  ...
                  >
                    <div
                      style={{ marginBottom: 6, fontWeight: 800, fontSize: 20 }}
                    >
                      Kết luận
                    </div>
                  ...
                  <>
                    ...
                    <div style={{ display: "flex", gap: 24, marginBottom: 12 }}>
                      <div style={{ flex: 1 }}>
                        <div>
                          Ngày xét nghiệm:{" "}
                          <b>{selectedOrder.appointmentDate || ""}</b>
                        </div>
                        <div>
                          Nhân viên lấy mẫu:{" "}
                          <b>
                            {selectedOrder.sampleInfo?.collector ||
                              selectedOrder.staffAssigned ||
                              ""}
                          </b>
                        </div>
                        <div>
                        ...
                        <div>
                          Địa chỉ hiện tại: <b>{selectedOrder.address || ""}</b>
                        </div>
                        <div
                          style={{
                            display: "flex",
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div>
                          Mã đơn hàng: <b>#{selectedOrder.id}</b>
                        </div>
                      </div>
                    </div>
                    <div
                                  ...
                                  <span style={{ fontWeight: 400 }}>
                                    {donor.idIssueDate
                                      ? dayjs(donor.idIssueDate).isValid()
                                        ? dayjs(donor.idIssueDate).format(
                                            "DD/MM/YYYY"
                                          )
                                        : donor.idIssueDate
                                      : ""}
                    ...
                    selectedOrder.resultTableData.length > 0 ? (
                      <div style={{ marginBottom: 24 }}>
                        ...
                        >
                          ...
                          >
                            ...
                            <tbody>
                              {selectedOrder.resultTableData.map(
                                (data, index) => (
                                  <tr
                                    key={data.key}
                                    style={{
                      ...
                      <div style={{ color: "#faad14", fontWeight: 600 }}>
                        Chưa có thông tin kết quả xét nghiệm
                      </div>
                    )}
                    <div
                      style={{
