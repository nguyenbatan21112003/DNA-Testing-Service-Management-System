...
const AdminTestResult = ({ isOpen, order, onClose }) => {
  if (!isOpen || !order || order.category !== 'admin') return null;
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 24, marginBottom: 12 }}>
          <div style={{ flex: 1 }}>
            <div>Ngày xét nghiệm: <b>{order.appointmentDate || ''}</b></div>
            <div>Nhân viên lấy mẫu: <b>{order.sampleInfo?.collector || order.staffAssigned || ''}</b></div>
            <div>Người yêu cầu xét nghiệm: <b>{order.name || ''}</b></div>
            <div>Địa chỉ hiện tại: <b>{order.address || ''}</b></div>
            <div style={{ display: 'flex', alignItems: 'center', marginTop: 2, flexWrap: 'nowrap' }}>
              ...
              <span style={{ whiteSpace: 'nowrap', overflowWrap: 'anywhere', width: '100%', display: 'inline-block', marginLeft: 8 }}>{order.type || ''}</span>
            </div>
          </div>
          <div style={{ flex: 1 }}>
            <div>Địa điểm lấy mẫu: <b>{getSampleMethodLabel(order.sampleMethod)}</b></div>
        ...
        {Array.isArray(order.sampleInfo?.donors) && order.sampleInfo.donors.length > 0 && (
          <div style={{ marginBottom: 24 }}>
            {order.sampleInfo.donors.map((donor, idx) => (
              <div key={idx} style={{ border: '1px solid #eee', borderRadius: 8, padding: 16, marginBottom: 12, background: '#fafbfc' }}>
                ...
                <div style={{ display: 'flex', gap: 0, marginBottom: 4 }}>
                  <div style={{ flex: 1, minWidth: 0 }}><b>Ngày cấp:</b> <span style={{ fontWeight: 400 }}>{
                    donor.idIssueDate ? (dayjs(donor.idIssueDate).isValid() ? dayjs(donor.idIssueDate).format('DD/MM/YYYY') : donor.idIssueDate) : ''
                  }</span></div>
              ...
              <table className="result-table" style={{ minWidth: 600, tableLayout: 'auto', borderCollapse: 'collapse', width: '100%' }}>
                ...
                <tbody>
                  {order.resultTableData.map((data, idx) => (
                    <tr key={data.key} style={{ background: idx % 2 === 0 ? '#fff' : '#f4f8ff' }}>
                      <td style={{ padding: '10px 14px', fontSize: 16, textAlign: 'center' }}>{idx + 1}</td>
                      <td style={{ padding: '10px 14px', fontSize: 16, textAlign: 'center' }}>{data.name}</td>
                      <td style={{ padding: '10px 14px', fontSize: 16, textAlign: 'center' }}>{data.birth || ''}</td>
